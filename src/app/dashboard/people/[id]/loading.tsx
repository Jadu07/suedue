export default function PersonProfileLoading() {
  return (
    <div className="max-w-5xl mx-auto space-y-6 animate-pulse">
      {/* Back Button Skeleton */}
      <div className="h-4 w-24 bg-[#333] rounded" />

      {/* Header Profile Card Skeleton */}
      <div className="bg-[#161616] border border-[#333] rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-full bg-[#222]" />
          <div className="space-y-2">
            <div className="h-6 w-48 bg-[#333] rounded" />
            <div className="flex gap-4">
              <div className="h-4 w-24 bg-[#222] rounded" />
              <div className="h-4 w-32 bg-[#222] rounded" />
            </div>
          </div>
        </div>
        <div className="space-y-2">
          <div className="h-3 w-16 bg-[#222] rounded sm:ml-auto" />
          <div className="h-8 w-32 bg-[#333] rounded" />
        </div>
      </div>

      {/* Tabs Skeleton */}
      <div className="flex gap-4 border-b border-[#333] pb-2">
        <div className="h-6 w-24 bg-[#333] rounded" />
        <div className="h-6 w-24 bg-[#222] rounded" />
        <div className="h-6 w-24 bg-[#222] rounded" />
      </div>

      {/* Content Skeleton */}
      <div className="bg-[#161616] border border-[#333] rounded-xl p-6 space-y-4">
        {[1, 2, 3].map((i) => (
          <div key={i} className="flex justify-between py-4 border-b border-[#333] last:border-0">
            <div className="space-y-2">
              <div className="h-5 w-40 bg-[#333] rounded" />
              <div className="h-4 w-24 bg-[#222] rounded" />
            </div>
            <div className="h-6 w-20 bg-[#222] rounded" />
          </div>
        ))}
      </div>
    </div>
  );
}
