...
export function MobileBottomNav() {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-canvas/95 backdrop-blur-md border-t border-hairline flex justify-around items-center p-2 pb-safe z-30 shadow-[0_-4px_16px_rgba(0,0,0,0.04)]">
      {navItems.map((item) => {
        const active = item.isActive(pathname);
        const Icon = item.icon;
        ...
        return (
          <Link
            ...
            href={item.href}
            prefetch={true}
            className={`flex flex-col items-center justify-center gap-1 py-1.5 px-3 rounded-xl transition-all select-none active:scale-90 ${
              active
                ? "text-ink font-black"
