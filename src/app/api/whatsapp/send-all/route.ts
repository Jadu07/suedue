...
export async function POST(req: NextRequest) {
  try {
    ...
    // Find all pending splits for this person
    const pendingSplits = await Split.find({ personId, status: { $in: ["PENDING", "PARTIALLY_PAID"] } }).populate("billId");
    ...
    if (totalRemainingPaise <= 0) {
      return NextResponse.json({ error: "No pending dues for this person (net balance is zero or credit)." }, { status: 400 });
    }
    ...
    await PaymentRequest.updateMany(
      { 
        ...
        $or: [
          { splitId: { $in: splitIds } },
          { splitIds: { $elemMatch: { $in: splitIds } } }
        ]
      },
      { $set: { status: "CANCELLED" } }
    );

    // Create consolidated payment request
    const payReq = await PaymentRequest.create({
      personId: person._id,
      splitIds: splitIds, // Consolidated!
